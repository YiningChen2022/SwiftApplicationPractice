...
class ViewFleaMarketTableViewController: UITableViewController ,UISearchResultsUpdating,UISearchBarDelegate{
    ...
    private func fetchAllPosts(){
        ...
        DatabaseManager.shared.getAllPosts{[weak self] posts in self?.posts = posts
            ...
            self?.posts=posts.filter{
                posttype in
                let scopeMatch=( posttype.type.lowercased().contains("Flea Market".lowercased()))
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "fleamarket"){
