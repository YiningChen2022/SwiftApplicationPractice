...
class CreateNewPsotViewController: UIViewController {
    ...
    @IBAction func didTapPosts(_ sender: Any) {
        ...
        // insert of post into DB
                ...
                let post=BlogPost(identifier:postId, title: title, timestamp: Date().timeIntervalSince1970, headerImageUrl:headerUrl, text: body, type: self.type, postUser: self.user?.name, postUserEmail: self.user?.email,postLikes: 0,postViews: 0)
